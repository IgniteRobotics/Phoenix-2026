...
public class VisionSubsystem extends SubsystemBase {
  ...
  public void periodic() {
    if (driveState.hasDriveStats()) {
      ...
      CameraConstants.photonPoseEstimator_FrontLeft.setReferencePose(driveStats.Pose);
      var cameraResults_FrontLeft = CameraConstants.photonCamera_FrontLeft.getAllUnreadResults();
      for (var result : cameraResults_FrontLeft) {
        Optional<EstimatedRobotPose> estimatedPose =
            CameraConstants.photonPoseEstimator_FrontLeft.update(result);
        if (!estimatedPose.isEmpty()) {
          evaluateMeasurement(estimatedPose.get(), CameraConstants.photonCameraName_FrontLeft);
        }
      }

      // camera 2
      CameraConstants.photonPoseEstimator_FrontRight.setReferencePose(driveStats.Pose);
      var cameraResults_FrontRight = CameraConstants.photonCamera_FrontRight.getAllUnreadResults();
      for (var result : cameraResults_FrontRight) {
        Optional<EstimatedRobotPose> estimatedPose =
            CameraConstants.photonPoseEstimator_FrontRight.update(result);
        if (!estimatedPose.isEmpty()) {
          evaluateMeasurement(estimatedPose.get(), CameraConstants.photonCameraName_FrontRight);
        }
      }
    }
  }
  ...
  public void setPipeline(int index) {
    CameraConstants.photonCamera_FrontLeft.setPipelineIndex(index);
    CameraConstants.photonCamera_FrontRight.setPipelineIndex(index);
  }
